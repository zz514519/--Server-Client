...
public class Server {
    ...
    public static void main(String[] args)throws IOException {
        ...
        try(DatagramSocket serverSocket = new DatagramSocket(PORT)) {       //构造数据报套接字并将其绑定到本地主机上的指定端口。
            ...
            while (true){
                ...
                DatagramPacket  packetFromClient = new DatagramPacket(receiveBuffer,0,receiveBuffer.length);    //构造一个 DatagramPacket用于接收长度的分组 length ，指定偏移到缓冲器中
                ...
                if(!meaningMap.containsKey(request)){
                    ...
                    response  = String.format(template,meaning,exampleSB.toString());   //使用指定的格式字符串和参数返回格式化的字符串
                }
                ...
                //1.6发送响应
                //如何获取客户端进程的ip + port
                InetAddress clientAddress = packetFromClient.getAddress();  //返回此 InetAddress对象的原始IP地址。
                int clientPort = packetFromClient.getPort();        //返回端口
                System.out.printf("DEBUG:：客户端的唯一标识是(%s:%d)%n",clientAddress.getHostAddress(),clientPort);

                byte[] responseBytes = response.getBytes(Server.CHARSET);   //将字符串转化为字节数组
                DatagramPacket packetToClient = new DatagramPacket(         //构造用于发送长度的分组数据报包 length具有偏移 ioffset指定主机上到指定的端口号
                        responseBytes, 0, responseBytes.length,         //要发送的数据
                        clientAddress,clientPort);          //要发送给端进程

                System.out.println("DEBUG：准备好了发送用的packet");
                serverSocket.send(packetToClient);          //从此套接字发送数据报包。
